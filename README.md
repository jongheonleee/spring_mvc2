...
- 3. 오류 코드와 메시지 처리
  ...
  - MessageCodesResolver는 검증 오류 코드로 메시지 코드를 생성함
  - 컨트롤러에서 검증 로직이 차지하는 비중이 커짐에 따라 별도의 클래스로 역할을 분리하는 것이 좋음
  - 또한 파라미터 앞에 @Validated 추가하면 자동으로 검증해줌(해당 애노테이션은 검증기를 실행하라는 의미를 갖음 - WebDataBinder에 등록한 검증기를 찾아서 실행함)
...
### 핵심 개념 및 목표
- 위의 검증 로직은 주로 특정 필드의 값이 빈 값인지 크기 범위를 벗어나는지 확인함, 즉 매우 일반적인 로직임
- 이를 공통화하고 표준화한 기술이 Bean Validation
- 쉽게 말하면 검증 애노테이션과 여러 인터페이스 집합
- 사용법은 밑에와 같음
  <br/>
  <img width="425" alt="image" src="https://github.com/jongheonleee/spring_mvc2/assets/87258372/100a3631-c1c8-4d26-ad86-9a4eecc9d1cc">
  - javax.validation은 자바 표준 인터페이스, org.hibernate.validatior는 하이버네이트 validator 구현체


## 📝 검증 기능 사용법

- 1. 스프링 MVC가 Bean Validation을 사용하는 원리
  - 스프링 부트가 spring-boot-starter-validation 라이브러리를 넣으면 자동으로 Bean Validation를 인지하고 스프링에 통합함
